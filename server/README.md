...
1、响应合理的状态码和数据
   HTTP状态码：
		...
		500（服务器内部错误） -> JS运行时错误，比如查询数据库出错，读取不存在的属性或变量，验证token出错
        401（未授权) -> 无token，token无效，token过期
        403（没权限，拒绝访问）-> 用户省份验证成功（token有效），但该用户无权操作该接口
        404（请求的资源/接口不存在）-> 像服务器请求的资源或接口服务器不存在
        405（请求路由正确，但请求方式错误）-> 请求地址是正确的，但不支持当前请求方式
        409（服务器处理请求时发生了冲突，如注册时用户名已存在）-> 信息冲突
		422（请求格式正确，但是由于含有语义错误）-> 请求时传的参数是正确的，但其类型或长度等不符合要求
