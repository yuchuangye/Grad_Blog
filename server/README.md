...
1、响应合理的状态码和数据
   HTTP状态码：
        ...
        500（服务器内部错误） -> JS运行时错误。比如操作数据库出错/读取不存在属性或变量/验证token出错 
   ...
   响应数据：
        数据结构：
            {
                code: 0|1|2  // 0代表业务逻辑成功, 1代表业务逻辑错误, 2也代表业务逻辑错误但该错误信息在前端不提示
                msg: 'xxxx'  // 请求完成提示文本
                data: {}|[]|''  // 服务器实际返回的数据, 可以是对象/数组/字符串等JS类型
            }
        200（无需手动抛出）：
        {
			code: 0,
        ...
        4xx
        （404无需手动抛出，其它需要 ctx.throw(4xx) 手动抛出，
		...
		抛出（405）请求方式错误时（服务器返回的数据只有一个字符串）： data: 'Method Not Allowed'
